...
The output `04_summary/*.read.summary` files summarize contact statistics and serve as QC metrics. The `03_dedup/*.barcode.cut_rank` files contain retained cell barcodes (2nd column) and their contact counts (1st column) after filtering. The `05_filtered/*.dedup.filtered.pairs.gz` files can be used in downstream pseudo-bulk or single-cell analysis.
